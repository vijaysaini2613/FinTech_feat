  ...
  return (
    <div className="glass-panel p-6 rounded-2xl mb-6">
      <div className="flex items-center space-x-3 mb-4">
        ...
        <div>
          <h3 className="font-bold text-slate-100 text-base">Next-Gen Payment Gateway Simulation Studio</h3>
          <p className="text-xs text-slate-400">Test Pay-by-Bank (UPI AutoPay), Programmable CBDC (e-Rupee), and Open Banking VRP rails live</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        {/* Preset 1: Tier 1 */}
        <div className="p-4 rounded-xl bg-slate-900/80 border border-blue-500/20 hover:border-blue-500/50 transition flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-2">
              <span className="font-bold text-xs text-blue-400 uppercase tracking-wider">Tier 1 Simulation</span>
              <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-blue-500/10 text-blue-300 border border-blue-500/20">
                Soft Failure
              </span>
            </div>
            <h4 className="font-bold text-sm text-white mb-1">HDFC Gateway Timeout</h4>
        ...
        <div className="p-4 rounded-xl bg-slate-900/80 border border-purple-500/20 hover:border-purple-500/50 transition flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-2">
              <span className="font-bold text-xs text-purple-400 uppercase tracking-wider">Pay-by-Bank (A2A)</span>
              <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-purple-500/10 text-purple-300 border border-purple-500/20">
                UPI AutoPay
              </span>
            </div>
            <h4 className="font-bold text-sm text-white mb-1">Expired Card e-Mandate</h4>
            <p className="text-xs text-slate-400 mb-3">
              Bypasses dead card network. Engine provisions 1-click UPI AutoPay mandate link & dispatches WhatsApp nudge.
            </p>
          </div>
          <button
            disabled={loading}
            onClick={() => handlePreset('TIER_2_MANDATE_EXPIRED')}
          ...
          >
            <Play className="w-3.5 h-3.5" />
            <span>Simulate Pay-by-Bank Rail</span>
          </button>
        </div>

        {/* Preset 3: Tier 3 (HITL Guardrail) */}
        <div className="p-4 rounded-xl bg-slate-900/80 border border-rose-500/20 hover:border-rose-500/50 transition flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-2">
              <span className="font-bold text-xs text-rose-400 uppercase tracking-wider">Tier 3 Guardrail</span>
              <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-rose-500/10 text-rose-300 border border-rose-500/20">
                High-Value HITL
              </span>
            </div>
            <h4 className="font-bold text-sm text-white mb-1">₹75,000 Invoice Failure</h4>
            <p className="text-xs text-slate-400 mb-3">
              Exceeds ₹25,000 safety threshold. Freezes automated execution and escalates to HITL dashboard queue.
            </p>
          </div>
          <button
            disabled={loading}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div className="p-4 rounded-xl bg-slate-900/80 border border-emerald-500/20 hover:border-emerald-500/50 transition flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className="p-2 rounded-xl bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
              <Coins className="w-5 h-5" />
            </div>
            <div>
              <h4 className="font-bold text-xs text-white">Programmable CBDC (e-Rupee) Rail</h4>
              <p className="text-[11px] text-slate-400">Zero-fee instant cross-border settlement for international mandate drops</p>
            </div>
          </div>
          <button
            disabled={loading}
        ...
        <div className="p-4 rounded-xl bg-slate-900/80 border border-amber-500/20 hover:border-amber-500/50 transition flex items-center justify-between">
          <div className="flex items-center space-x-3">
            <div className="p-2 rounded-xl bg-amber-500/10 text-amber-400 border border-amber-500/20">
              <Terminal className="w-5 h-5" />
            </div>
            <div>
              <h4 className="font-bold text-xs text-white">Open Banking VRP Rail</h4>
              <p className="text-[11px] text-slate-400">Variable Recurring Payments bypassing card network interchange fees</p>
            </div>
          </div>
          <button
            disabled={loading}
      ...
      <form onSubmit={handleCustomTrigger} className="p-4 rounded-xl bg-slate-900/50 border border-slate-800">
        <div className="flex items-center justify-between mb-3">
          <span className="font-bold text-xs text-slate-200 flex items-center space-x-1">
            <Sparkles className="w-3.5 h-3.5 text-cyan-400" />
            <span>Evaluate Gemini 2.5 Flash AI Classifier with Custom Raw Error Text</span>
          </span>
          <span className="text-[11px] text-slate-400">Zero AI execution decisions safety boundary</span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
          <div>
            ...
            <input
              ...
              className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-1.5 text-slate-100 text-xs font-mono focus:border-cyan-500 focus:outline-none"
            />
          </div>

          <div>
            <label className="text-xs text-slate-400 block mb-1">Raw Error Code:</label>
            <input
              type="text"
          ...
          <div>
            <label className="text-xs text-slate-400 block mb-1">Raw Bank Gateway Error String:</label>
            <input
              type="text"
